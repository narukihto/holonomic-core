...
pub fn calculate_jacobian_manifold_operator(path: &[usize], tension: &TensionMatrix) -> Vec<Vec<f64>> {
    let n = tension.size;
    let mut jacobian = vec![vec![0.0; n]; n];
    if path.len() < 2 { return jacobian; }
        ...
        for j in 0..n {
            if i != j && i < path.len() && j < path.len() {
                ...
                if u < tension.data.len() && v < tension.data[u].len() {
                    ...
                    jacobian[i][j] = if metric_val > 1e-9 { 1.0 / metric_val } else { 1e6 };
                }
            }
        }
    }
